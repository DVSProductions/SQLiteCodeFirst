...
namespace SQLite.CodeFirst.Builder {
	internal class CreateDatabaseStatementBuilder : IStatementBuilder<CreateDatabaseStatement> {
		protected readonly EdmModel edmModel;
		protected readonly Collation defaultCollation;
		...
		protected IEnumerable<CreateTableStatement> GetCreateTableStatements() {
		...
		protected IEnumerable<CreateIndexStatementCollection> GetCreateIndexStatements() {
			foreach (var entitySet in edmModel.Container.EntitySets) {
